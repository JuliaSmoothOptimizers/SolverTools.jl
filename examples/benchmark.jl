...
function two_solvers()
  ...
  bmark_args = Dict{Symbol, Any}(:format => :jump, :skipif => model -> model.meta.ncon > 0)
  profile_args = Dict{Symbol, Any}(:title => title)
  profiles = bmark_and_profile(solvers, jump_probs, n, bmark_args=bmark_args, profile_args=profile_args)
end
